  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Tedarikçi Skorlama</h1>
          <p className="text-gray-500 mt-1">Atölye performans değerlendirmesi</p>
        </div>
      ...
      {scores.length > 0 ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-right text-gray-500 font-medium">Kalite</th>
                <th className="px-4 py-3 text-right text-gray-500 font-medium">Teslimat</th>
                <th className="px-4 py-3 text-right text-gray-500 font-medium">Maliyet</th>
                <th className="px-4 py-3 text-right text-gray-500 font-medium">Uyumluluk</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {scores.map((s, i) => {
                const tc = s.tier ? tierColor(s.tier) : { bg: 'bg-gray-100', text: 'text-gray-500' }
                return (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <span className="text-[#197A56] font-medium">{s.workshop_code}</span>
                      <span className="text-gray-500 ml-2 text-xs">{s.workshop_name}</span>
                    </td>
                    <td className="px-4 py-3 text-center text-gray-600">{s.year}/{s.month}</td>
                    ...
                    <td className="px-4 py-3 text-right font-bold text-gray-900">{s.composite_sc ?? '—'}</td>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-8 text-center">
          <p className="text-gray-600">Henüz skor hesaplanmamış</p>
          <p className="text-sm text-gray-400 mt-1">Önce üretim ve kalite verisi girin, sonra skorları hesaplayın</p>
        </div>
      )}
    </div>
  )

...
export default function WorkforcePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">İşgücü Devir Oranı</h1>
        <p className="text-gray-500 mt-1">Aylık personel giriş-çıkış takibi</p>
      </div>
      ...
      {workshopId && (
        <>
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Personel Verileri</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Toplam Çalışan (ay sonu)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">İşten Ayrılan</label>
                <input type="number" className={inputClass} value={form.left_count} onChange={e => setForm(p => ({...p, left_count: parseInt(e.target.value)||0}))} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">İşe Başlayan</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Isınma Sürecindeki</label>
                <input type="number" className={inputClass} value={form.in_warmup} onChange={e => setForm(p => ({...p, in_warmup: parseInt(e.target.value)||0}))} />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Ort. Kıdem (ay)</label>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200 grid grid-cols-3 gap-4">
              <div className={`rounded-lg p-3 text-center ${netChange >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                <p className="text-xs text-gray-500">Net Değişim</p>
              ...
              <div className="bg-blue-50 rounded-lg p-3 text-center">
                <p className="text-xs text-gray-500">Isınma Sürecinde</p>

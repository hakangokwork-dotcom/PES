...
const durumColor: Record<string, string> = { PLAN: 'bg-blue-100 text-blue-800', UYGULA: 'bg-amber-100 text-amber-800', KONTROL: 'bg-purple-100 text-purple-800', STANDART: 'bg-green-100 text-green-800', IPTAL: 'bg-gray-100 text-gray-500' }
const durumLabel: Record<string, string> = { PLAN: 'Plan', UYGULA: 'Uygula', KONTROL: 'Kontrol', STANDART: 'Standart', IPTAL: 'Iptal' }

export default function Wrapper() {
  return <Suspense fallback={<div className="p-6 text-gray-400">Yukleniyor...</div>}><KaizenPage /></Suspense>
}
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kaizen / PDCA</h1>
          <p className="text-sm text-gray-500">Surekli iyilestirme aksiyonlari ve takibi</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <h2 className="font-semibold text-gray-900">Yeni Iyilestirme Aksiyonu</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div className="md:col-span-2"><label className="block text-xs text-gray-600 mb-1">Baslik *</label><input required className={ic} value={form.baslik} onChange={e => setForm(p => ({...p, baslik: e.target.value}))} placeholder="Darbogaz operasyonu icin ek makine" /></div>
            <div><label className="block text-xs text-gray-600 mb-1">Kategori</label><select className={ic} value={form.kategori} onChange={e => setForm(p => ({...p, kategori: e.target.value}))}>{KATEGORILER.map(k => <option key={k} value={k}>{k}</option>)}</select></div>
            <div><label className="block text-xs text-gray-600 mb-1">Hedef Metrik</label><input className={ic} value={form.hedef_metrik} onChange={e => setForm(p => ({...p, hedef_metrik: e.target.value}))} placeholder="orn. Cikti Verimliligi %" /></div>
            <div><label className="block text-xs text-gray-600 mb-1">Mevcut Deger</label><input type="number" step="0.1" className={ic} value={form.mevcut_deger} onChange={e => setForm(p => ({...p, mevcut_deger: e.target.value}))} /></div>
            <div><label className="block text-xs text-gray-600 mb-1">Hedef Deger</label><input type="number" step="0.1" className={ic} value={form.hedef_deger} onChange={e => setForm(p => ({...p, hedef_deger: e.target.value}))} /></div>
            <div><label className="block text-xs text-gray-600 mb-1">Sorumlu</label><input className={ic} value={form.sorumlu} onChange={e => setForm(p => ({...p, sorumlu: e.target.value}))} /></div>
            <div><label className="block text-xs text-gray-600 mb-1">Baslangic</label><input type="date" className={ic} value={form.baslangic_tarihi} onChange={e => setForm(p => ({...p, baslangic_tarihi: e.target.value}))} /></div>
            <div><label className="block text-xs text-gray-600 mb-1">Bitis</label><input type="date" className={ic} value={form.bitis_tarihi} onChange={e => setForm(p => ({...p, bitis_tarihi: e.target.value}))} /></div>
          </div>
      ...
      {/* Aksiyon Listesi */}
      {actions.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl p-8 text-center text-gray-400">Henuz kaizen aksiyonu yok</div>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded bg-gray-100 text-gray-600">{a.kategori}</span>
                    <span className="font-medium text-gray-900">{a.baslik}</span>
                  </div>
                  {a.hedef_metrik && (
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">{a.sorumlu || '---'} | {a.baslangic_tarihi ? `${a.baslangic_tarihi} → ${a.bitis_tarihi || '...'}` : '---'}</p>
                </div>

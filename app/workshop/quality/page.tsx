...
export default function QualityWrapper() {
  return <Suspense fallback={<div className="p-6 text-gray-400">Yükleniyor...</div>}><WorkshopQualityPage /></Suspense>
}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">{workshopName} — Kalite Girişi</h1>
      </div>

      <div className="bg-white border border-gray-200 rounded-xl p-4 flex gap-4 items-end flex-wrap">
        <div><label className="block text-xs font-medium text-gray-600 mb-1">Yil</label><select className="px-3 py-2 border border-gray-300 rounded-lg text-sm" value={year} onChange={e => setYear(parseInt(e.target.value))}><option value={2025}>2025</option><option value={2026}>2026</option></select></div>
        ...
        <div><label className="block text-xs font-medium text-gray-600 mb-1">Bant</label>
          <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm" value={form.line_id} onChange={e => setForm(p => ({...p, line_id: e.target.value}))}>
            <option value="">Tumu</option>
            {lines.map(l => <option key={l.id} value={l.id}>{l.code} - {l.name}</option>)}
          </select>
        </div>
        <div><label className="block text-xs font-medium text-gray-600 mb-1">Model Kodu</label>
          <input className="px-3 py-2 border border-gray-300 rounded-lg text-sm w-32" value={form.model_code} onChange={e => setForm(p => ({...p, model_code: e.target.value}))} placeholder="orn. WASHA" />
        </div>
      </div>

      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Kontrol Edilen</label><input type="number" className={ic} value={form.inspected_qty} onChange={e => setForm(p => ({...p, inspected_qty: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">İlk Geçiş (FPQ)</label><input type="number" className={ic} value={form.first_pass_qty} onChange={e => setForm(p => ({...p, first_pass_qty: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Red Edilen</label><input type="number" className={ic} value={form.rejected_qty} onChange={e => setForm(p => ({...p, rejected_qty: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Yeniden İşlem</label><input type="number" className={ic} value={form.rework_qty} onChange={e => setForm(p => ({...p, rework_qty: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Müşteri İade</label><input type="number" className={ic} value={form.customer_return} onChange={e => setForm(p => ({...p, customer_return: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">En Sık Hata</label>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 grid grid-cols-3 gap-4">
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            <p className="text-xs text-gray-500">Hedef FPQ</p>
            <p className="text-2xl font-bold text-gray-400">≥%95</p>
          </div>
        </div>
      </div>

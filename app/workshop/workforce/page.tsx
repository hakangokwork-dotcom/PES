...
export default function WorkforceWrapper() {
  return <Suspense fallback={<div className="p-6 text-gray-400">Yükleniyor...</div>}><WorkshopWorkforcePage /></Suspense>
}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">{workshopName} — İşgücü</h1>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Toplam Çalışan</label><input type="number" className={ic} value={form.total_staff} onChange={e => setForm(p => ({...p, total_staff: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Ayrılan</label><input type="number" className={ic} value={form.left_count} onChange={e => setForm(p => ({...p, left_count: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Başlayan</label><input type="number" className={ic} value={form.joined_count} onChange={e => setForm(p => ({...p, joined_count: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Isınma Sürecinde</label><input type="number" className={ic} value={form.in_warmup} onChange={e => setForm(p => ({...p, in_warmup: parseInt(e.target.value)||0}))} /></div>
          <div><label className="block text-xs font-medium text-gray-600 mb-1">Ort. Kıdem (ay)</label><input type="number" className={ic} value={form.avg_tenure_mon} onChange={e => setForm(p => ({...p, avg_tenure_mon: parseFloat(e.target.value)||0}))} step={0.1} /></div>
          <div className={`rounded-lg p-3 text-center ${Number(turnoverRate) <= 20 ? 'bg-green-50' : Number(turnoverRate) <= 40 ? 'bg-amber-50' : 'bg-red-50'}`}>
            <p className="text-xs text-gray-500">Devir Oranı</p>
      ...
      <button onClick={handleSave} disabled={loading} className="px-6 py-2.5 bg-[#197A56] text-white rounded-lg hover:bg-[#0E3E1B] text-sm font-medium disabled:opacity-50">{loading ? 'Kaydediliyor...' : 'Kaydet'}</button>
    </div>
  )
}

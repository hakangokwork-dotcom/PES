...
export default function Wrapper() {
  return <Suspense fallback={<div className="p-6 text-gray-400">Yukleniyor...</div>}><YikamaUkpPage /></Suspense>
}
...
function YikamaUkpPage() {
  ...
  const ic = 'w-full border border-gray-300 rounded-lg px-3 py-2 text-sm'

  if (!wid) return <div className="p-6 text-gray-400">Atolye secin</div>

  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Yikama & UKP Takibi</h1>

      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit">
        <button onClick={() => setTab('yikama')} className={`px-4 py-2 rounded-md text-sm font-medium ${tab === 'yikama' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}>Yikama</button>
        <button onClick={() => setTab('ukp')} className={`px-4 py-2 rounded-md text-sm font-medium ${tab === 'ukp' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}>UKP</button>
      </div>

      {tab === 'yikama' && (
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <h2 className="font-semibold text-gray-900 mb-3">Yikama Kaydi Ekle</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div><label className="block text-xs text-gray-600 mb-1">Tarih</label><input type="date" className={ic} value={yForm.tarih} onChange={e => setYForm(p => ({...p, tarih: e.target.value}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Giren Adet</label><input type="number" className={ic} value={yForm.giren_adet || ''} onChange={e => setYForm(p => ({...p, giren_adet: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Cikan Adet</label><input type="number" className={ic} value={yForm.cikan_adet || ''} onChange={e => setYForm(p => ({...p, cikan_adet: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Hatali</label><input type="number" className={ic} value={yForm.hatali_adet || ''} onChange={e => setYForm(p => ({...p, hatali_adet: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Cevrim Sayisi</label><input type="number" className={ic} value={yForm.cevrim_sayisi || ''} onChange={e => setYForm(p => ({...p, cevrim_sayisi: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Cevrim Sure (dk)</label><input type="number" className={ic} value={yForm.cevrim_sure_dk || ''} onChange={e => setYForm(p => ({...p, cevrim_sure_dk: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Enerji (kWh)</label><input type="number" className={ic} value={yForm.enerji_kwh} onChange={e => setYForm(p => ({...p, enerji_kwh: e.target.value}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Su (litre)</label><input type="number" className={ic} value={yForm.su_litre} onChange={e => setYForm(p => ({...p, su_litre: e.target.value}))} /></div>
            </div>
      ...
      {tab === 'ukp' && (
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <h2 className="font-semibold text-gray-900 mb-3">UKP Kaydi Ekle</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div><label className="block text-xs text-gray-600 mb-1">Tarih</label><input type="date" className={ic} value={uForm.tarih} onChange={e => setUForm(p => ({...p, tarih: e.target.value}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Utu Adet</label><input type="number" className={ic} value={uForm.utu_adet || ''} onChange={e => setUForm(p => ({...p, utu_adet: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Kontrol Adet</label><input type="number" className={ic} value={uForm.kontrol_adet || ''} onChange={e => setUForm(p => ({...p, kontrol_adet: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Paket Adet</label><input type="number" className={ic} value={uForm.paket_adet || ''} onChange={e => setUForm(p => ({...p, paket_adet: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Hatali</label><input type="number" className={ic} value={uForm.hatali_adet || ''} onChange={e => setUForm(p => ({...p, hatali_adet: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Personel</label><input type="number" className={ic} value={uForm.personel_sayisi || ''} onChange={e => setUForm(p => ({...p, personel_sayisi: Number(e.target.value)}))} /></div>
              <div><label className="block text-xs text-gray-600 mb-1">Calisma (dk)</label><input type="number" className={ic} value={uForm.calisma_dk || ''} onChange={e => setUForm(p => ({...p, calisma_dk: Number(e.target.value)}))} /></div>
            </div>

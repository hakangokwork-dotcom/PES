...
export default function AtolyeProfilTablo({
  ...
  const kutular: { anahtar: DenetimDurum; etiket: string; renk: string }[] = [
    ...
    { anahtar: 'YOK', etiket: 'Denetim kaydı yok', renk: 'text-faint' },
    ...
    <div className="space-y-4">
        ...
        {kutular.map((k) => (
          <button
            ...
            className={`text-left bg-white border rounded-xl p-4 transition-colors ${
              durumFiltre === k.anahtar ? 'border-accent ring-1 ring-accent' : 'border-line-soft hover:bg-canvas'
            }`}
          >
            <div className={`text-2xl font-bold ${k.renk}`}>{sayim[k.anahtar]}</div>
            <div className="text-xs text-faint mt-0.5">{k.etiket}</div>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <input
          ...
          className="flex-1 min-w-[220px] px-3 py-2 border border-line rounded-lg text-sm"
        />
        <select
          value={tip}
        ...
        <select
          ...
          className="px-3 py-2 border border-line rounded-lg text-sm bg-white max-w-[220px]"
        >
      ...
      <div className="bg-white border border-line-soft rounded-xl overflow-x-auto">
        <table className="w-full text-sm min-w-[1100px]">
          <thead>
            <tr className="bg-canvas border-b border-line-soft">
              ...
              <th className="px-4 py-3 text-left text-faint font-medium">Tedarik müd.</th>
              <th className="px-4 py-3 text-left text-faint font-medium">FKU</th>
              <th className="px-4 py-3 text-left text-faint font-medium">WKYS</th>
              <th className="px-4 py-3 text-left text-faint font-medium">Sosyal uygunluk</th>
            ...
            {filtreli.map((s) => (
              <tr key={s.id} className={`hover:bg-canvas transition-colors ${s.is_active ? '' : 'opacity-50'}`}>
                <td className="px-4 py-3">
                  <Link href={`/pes/workshops/${s.id}`} className="text-accent font-medium hover:underline">
                    {s.code}
                  </Link>
                </td>
                <td className="px-4 py-3 text-ink max-w-[220px]">
                  ...
                  {!s.profil_var && (
                    <div className="text-xs text-faint">künye boş</div>
                  )}
                </td>
                <td className="px-4 py-3 text-muted text-xs max-w-[150px] truncate">
                  {s.tedarik_mudurlugu ?? '—'}
                </td>
                <td className="px-4 py-3 text-muted text-xs max-w-[140px] truncate">{s.fku ?? '—'}</td>
                ...
                <td className="px-4 py-3">
                  ...
                  {s.wkys_tarih && (
                    <div className="text-xs text-faint mt-1">
                      ...
                      <span className="text-faint"> → {tarihTR(s.wkys_sonraki)}</span>
                    </div>
                  )}
                </td>
                ...
                <td className="px-4 py-3">
                  ...
                  {s.sosyal_tarih && (
                    <div className="text-xs text-faint mt-1">
                      ...
                      <span className="text-faint"> → {tarihTR(s.sosyal_sonraki)}</span>
                    </div>
                  )}
                </td>
        ...
        {filtreli.length === 0 && (
          <p className="px-4 py-8 text-center text-sm text-faint">Filtreye uyan atölye yok.</p>
        )}
      </div>
    </div>
  )
